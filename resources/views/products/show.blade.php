                ...
                <div>
                    <div class="relative border border-slate-200 bg-white p-2 shadow-sm">
                        <div class="flex aspect-square items-center justify-center bg-white p-6">
                            <img data-main-image src="{{ frontend_image_value($product['image']) }}" alt="{{ $product['name'] }}"
                    ...
                    <div class="mt-3 grid grid-cols-4 gap-3">
                        @foreach ($product['gallery'] as $image)
                            <button type="button" data-thumb="{{ frontend_image_value($image) }}"
                                class="aspect-square overflow-hidden border-2 bg-white p-1 transition-colors @if ($loop->first) border-emerald-600 @else border-slate-200 hover:border-emerald-300 @endif"
                                aria-label="View image {{ $loop->iteration }}">
                                <img src="{{ frontend_image_value($image) }}" alt="{{ $product['name'] }} image {{ $loop->iteration }}" loading="lazy"
                                    class="h-full w-full object-contain">
                            </button>
                        @endforeach
                    </div>
