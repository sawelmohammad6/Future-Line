...
<article class="group flex h-full flex-col overflow-hidden border border-slate-200 bg-white shadow-sm transition duration-200 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-950/5">
    <div class="relative bg-white">
        <div class="flex h-52 items-center justify-center bg-white p-4">
            <img src="{{ frontend_image_value($material['image'] ?? '') }}" alt="{{ $material['name'] ?? 'Raw material' }}" loading="lazy" decoding="async" class="h-full w-full object-contain">
        </div>
    ...
    <div class="flex flex-1 flex-col p-4">
        ...
        <a href="{{ route('products.show', $material['slug'] ?? 'raw-material') }}" class="mt-4 inline-flex items-center justify-center bg-emerald-700 px-3 py-2.5 text-xs font-bold text-white transition-colors hover:bg-emerald-800">{{ __('common.explore_products') }}</a>
    </div>
</article>
