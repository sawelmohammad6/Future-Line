@props(['partner' => []])
@php($partnerImage = frontend_image_url('importers_exporters.' . ($partner['image_key'] ?? '')))
    ...
    <div class="flex-1">
        <div class="flex items-start justify-between gap-4">
            <div class="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden border border-slate-200 bg-white p-1.5">
                @if ($partnerImage)
                    <img src="{{ $partnerImage }}" alt="{{ $partner['name'] ?? 'Importer / Exporter' }} logo" loading="lazy" decoding="async" class="h-full w-full object-contain">
                @else
                    <span class="text-sm font-bold text-emerald-800">{{ $partner['logo'] ?? 'FT' }}</span>
                @endif
            </div>
