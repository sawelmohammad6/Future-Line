...
<article class="group flex h-full flex-col overflow-hidden border border-slate-200 bg-white shadow-sm transition duration-200 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-950/5">
    <div class="relative aspect-[4/3] overflow-hidden bg-slate-100">
        <img src="{{ $product['image'] ?? '' }}" alt="{{ $product['name'] ?? 'Product' }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105">
    ...
    <div class="flex flex-1 flex-col p-4">
        <div class="flex items-center justify-between gap-2">
            ...
            <span class="text-xs font-medium text-emerald-700">In Stock</span>
        </div>
        <h3 class="mt-3 text-base font-bold leading-6 text-slate-950">{{ $product['name'] ?? 'Product name' }}</h3>
        <p class="mt-1 text-xs font-medium text-slate-500">{{ $product['grade'] ?? 'Standard grade' }} <span class="px-1 text-slate-300">|</span> {{ $product['seller'] ?? 'Verified supplier' }}</p>
        <div class="mt-4 flex items-end justify-between gap-3 border-y border-slate-100 py-3">
            <div><p class="text-lg font-bold text-slate-950">{{ $product['price'] ?? 'Price on request' }}</p><p class="mt-0.5 text-xs text-slate-500">MOQ: {{ $product['moq'] ?? '-' }}</p></div>
