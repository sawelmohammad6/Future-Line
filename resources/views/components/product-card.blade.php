...
<article class="group flex h-full flex-col overflow-hidden border border-slate-200 bg-white shadow-sm transition duration-200 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-950/5">
    <div class="product-image relative bg-white">
        <div class="flex h-52 items-center justify-center bg-white p-4">
            <img src="{{ frontend_image_value($product['image'] ?? '') }}" alt="{{ $product['name'] ?? 'Product' }}" loading="lazy" decoding="async" class="h-full w-full object-contain">
        </div>
    ...
    <div class="flex flex-1 flex-col p-4">
        ...
        <div class="mt-4 grid grid-cols-2 gap-2">
            <a href="{{ route('products.show', $product['slug'] ?? 'product') }}" class="inline-flex items-center justify-center border border-slate-300 px-3 py-2.5 text-xs font-bold text-slate-700 transition-colors hover:border-emerald-600 hover:text-emerald-700">{{ __('common.view_product') }}</a>
