@props(['company' => []])
@php($companyImage = frontend_image_url('companies.' . ($company['image_key'] ?? '')))
<article class="flex h-full flex-col border border-slate-200 bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-950/5">
    <div class="flex-1">
        <div class="flex h-20 w-20 items-center justify-center overflow-hidden border border-slate-200 bg-white p-1.5">
            @if ($companyImage)
                <img src="{{ $companyImage }}" alt="{{ $company['name'] ?? 'Company' }} logo" loading="lazy" decoding="async" class="h-full w-full object-contain">
            @else
                <span class="text-lg font-bold tracking-tight text-slate-800">{{ $company['logo'] ?? 'FT' }}</span>
            @endif
        </div>
